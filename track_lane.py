...
from ultralytics import SAM  # Import the SAM model from ultralytics
...
model = YOLO('D:/Autonomus_Vehicle/best.pt')  # Update with actual path
...
cv2.namedWindow("FastSAM - Press Q to Quit", cv2.WINDOW_NORMAL)
cv2.namedWindow("Masked Lane Region", cv2.WINDOW_NORMAL)
...
while cap.isOpened():
    ...
    # Annotated frame with segmentation overlays
    ...
    # Alpha blending for the annotated frame
    ...
    # Isolated lane region with centerline and vertical reference line
    ...
    # Calculate lane offset and steering adjustment
    if centerline:
        ...
        # Calculate lane angle and steering adjustment
        coeffs = np.polyfit([pt[1] for pt in centerline], centerline_x, 1)
        slope, intercept = coeffs
        lane_angle = np.arctan(slope) * (180 / np.pi)  # Convert to degrees

        # PID control for steering adjustment
        delta_time = time.time() - start_time
        pwm_value = pid_controller.calculate(error_normalised, delta_time)

        # Print numerical details in console
        print(f"Lane Offset: {average_offset:.2f} pixels")
        print(f"Lane Angle: {lane_angle:.2f} degrees")
        print(f"Steering Adjustment (PWM): {pwm_value:.2f}")
        print(f"Error Normalized: {error_normalised:.2f}")
    else:
        print("No centerline detected, skipping steering adjustment.")
        error_normalised = 0
        lane_angle = 0
        pwm_value = 0

    # Annotate the annotated_frame with offset and PWM
    cv2.putText(annotated_frame, f"Error: {error_normalised:.2f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255),
                2)
    cv2.putText(annotated_frame, f"PWM: {pwm_value:.2f}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

    # Draw the lane centerline points on the annotated frame
    if centerline:
        points = np.array(centerline, dtype=np.int32).reshape((-1, 1, 2))
        cv2.polylines(masked_frame, [points], isClosed=False, color=(0, 255, 0), thickness=2)

    # Draw the vertical reference line on the masked lane region
    center_x = width // 2
    cv2.line(masked_frame, (center_x, 0), (center_x, height), (0, 0, 255), 2)

    # Display both windows
    cv2.imshow("FastSAM - Press Q to Quit", annotated_frame)
    cv2.imshow("Masked Lane Region", masked_frame)
